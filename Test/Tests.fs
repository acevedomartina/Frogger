...
[<Test>]
member this.``Final function should return Error if game state is Error``() =
    // Arrange
    let errorMessage = "Game Over"
    let gameResult = Error errorMessage
    ...
    | Error e -> e |> should equal errorMessage

[<Test>]
member this.``Final function should call CheckWin if game state is Ok``() =
    // Arrange
    let initialGameState = {
        Player = { PosX = WIDTH / 2; PosY = One; Width = 50 }
        Fondo = { Obstacles = Map.empty }
    ...
    let gameResult = Ok initialGameState
    let mutable checkWinCalled = false

    // Mock CheckWin to detect if it's called
